...
export default function ProductIssuePage() {
  ...
  const issueId = usePathname().split("/")[4];

  // check if id is a number and issueId is a number
  // if not, redirect to 404
  if (isNaN(Number(id)) || isNaN(Number(issueId))) {
    redirect("/404");
  }
          ...
          <div className="flex flex-row p-8 w-full">
            ...
            }
            {
              <div className="flex flex-col w-full">
                {reviews.map((review, index) => (
                  <div key={index} className="flex flex-row w-ful">
                    <div className="text-xl w-[600px]" > {review.review} </div>
                    {/* rating stars */}
                    {[...Array(5)].map((_, index) => (
                      index < review.rating ?
