...
export function ProductIssuePage({ name }: { name: string }) {
  ...
  return (
    <>
      <main className="flex min-h-screen flex-row w-full p-8">
        <div className="flex min-h-screen flex-col w-[80%]">
        ...
        {/* <WordCloudUI data={ clusters.map(cluster => ({ text: cluster.name, value: cluster.item_count }))}></WordCloudUI> */}
        <div className="min-h-screen w-[20%]">
          <WordCloudUI data = {data}></WordCloudUI>
        </div>
      </main>
    </>
  );
