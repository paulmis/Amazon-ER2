...
def get_preprocessed_dataset(
    min_reviews=10, exclude_5_star=True, min_review_length=20, max_review_length=5000
):
    ...
    hash = str(
        hashlib.sha256(
            str(min_reviews).encode("utf-8")
            + str(exclude_5_star).encode("utf-8")
            + str(min_review_length).encode("utf-8")
            + str(max_review_length).encode("utf-8")
        ).hexdigest()
    )
    ...
    reviews_df = reviews_df.groupby("Product Name").filter(
    ...
    preprocessed_dataset.to_csv(
        "data/preprocessed_dataset_" + hash + ".csv", index=False
    )
...
def process_product_reviews(reviews_df, products):
    ...
    for product in products:
        ...
        number_of_reviews_to_sample = get_number_of_reviews_to_sample(
            len(product_reviews)
        )
        ...
        product_reviews.loc[:, "Length of Reviews"] = product_reviews[
            "Reviews"
        ].str.len()
        product_reviews.loc[:, "Review Votes"] = (
            product_reviews["Review Votes"].fillna(0).astype(int)
        )
...
def pareto_frontier(df, n):
    ...
    sorted_df = df.sort_values(
        by=["Review Votes", "Length of Reviews"], ascending=[False, False]
    )
    ...
    for i in range(len(sorted_df)):
        ...
        for j in pareto_frontier_indices:
            ...
            if (
                selected_row["Review Votes"] >= row["Review Votes"]
                and selected_row["Length of Reviews"] >= row["Length of Reviews"]
            ):
                dominated = True
                break
